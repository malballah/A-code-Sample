...
;(function ($, window, undefined)
{
    ...
    */
        ...
        $.each(this.columns, function (index, column)
        {
            if (column.visible)
            {
                ...
                html += tpl.headerCell.resolve(getParams.call(that, {
                    column: column, icon: icon,title:column.title, sortable: sorting && column.sortable && css.sortable || "",
    ...
    Grid.defaults = {
         ...
         * @since 1.1.0
         **/
         //Mahmoud Al ballah:change default ajax request to match EMS needs
        requestHandler: function (request) {
            request.currentPage = request.current;
            request.pageSize = request.rowCount;
            var sortStr = JSON.stringify(request.sort);
            if (sortStr !== "{}") {
                request.sortField = sortStr.split(":")[0].replace(/[",{]/g, '');
                request.sortType = sortStr.split(":")[1].replace(/[",}]/g, '');
            }
         ...
         * @since 1.1.0
         **/
        //Mahmoud Al ballah:change default response to match EMS needs
        responseHandler: function (response) {
            if(response.rowCount===undefined)
                response.rowCount = response.PageSize || response.pageSize;
            if (response.current === undefined)
                response.current = response.CurrentPage || response.currentPage;
            return response;
        },
        ...
        templates: {
            ...
            headerCell: "<th data-column-id=\"{{ctx.column.id}}\" title=\"{{ctx.title}}\" class=\"{{ctx.css}}\" style=\"{{ctx.style}}\"><a href=\"javascript:void(0);\" class=\"{{css.columnHeaderAnchor}} {{ctx.sortable}}\"><span class=\"{{css.columnHeaderText}}\">{{ctx.column.text}}</span>{{ctx.icon}}</a></th>",
